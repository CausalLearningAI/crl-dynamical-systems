...
class ODESYSLP(nn.Module):
    ...
    def build_constraints(self):
        ...
        eq_A = torch.sparse_coo_tensor(
            [
                self.row_dict[ConstraintType.Equation],
                self.col_dict[ConstraintType.Equation],
            ],
        ...
        if self.n_iv > 0:
            initial_A = torch.sparse_coo_tensor(
                [
                    self.row_dict[ConstraintType.Initial],
                    self.col_dict[ConstraintType.Initial],
                ],
        ...
        derivative_A = torch.sparse_coo_tensor(
            [
                self.row_dict[ConstraintType.Derivative],
                self.col_dict[ConstraintType.Derivative],
            ],
    ...
    def get_row_col_sorted_indices(self, row, col, exclude_eps=True):
        # Compute indices sorted by row and column and repeats. Useful for sparse outer product
        # when computing constraint derivatives
