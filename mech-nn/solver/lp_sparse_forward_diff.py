...
class ODESYSLP(nn.Module):
        ...
        for step in range(self.n_iv_steps):
            for dim in range(self.n_dim):
                for i in range(self.n_iv):
                    # self.add_constraint(var_list = [(0,dim,i)], values=[1], rhs=Const.PH,
                    # constraint_type=ConstraintType.Initial)
                    self.add_constraint(
                        var_list=[(step, dim, i)], values=[1], rhs=Const.PH, constraint_type=ConstraintType.Initial
                    )
                    # self.add_constraint(var_list = [(1,dim,i)], values=[1], rhs=Const.PH,
                    # constraint_type=ConstraintType.Initial)
        ...
        if self.periodic_boundary:
            for dim in range(self.n_dim):
                for order in range(self.n_order - 1):
                    self.add_constraint(
                        var_list=[(0, dim, order), (self.n_step - 1, dim, order)],
                        values=[1, -1],
                        rhs=Const.PH,
                        constraint_type=ConstraintType.Initial,
                    )
    ...
    def build_equation_constraints(self):
        ...
        for e in range(self.n_equations):
            for step in range(self.n_step):
                ...
                self.add_constraint(
                    var_list=var_list, values=val_list, rhs=Const.PH, constraint_type=ConstraintType.Equation
                )
            ...
            for step in range(1, self.n_step - 1):
                for dim in range(self.n_system_vars):
                    ...
                    self.add_constraint(
                        var_list=[
                            VarType.EPS,
                            (step - 1, dim, var_order - 1),
                            (step, dim, var_order - 1),
                            (step + 1, dim, var_order - 1),
                            (step, dim, var_order),
                        ],
                        # values= [ 1,            -0.5/h,                0,
            ...
            for step in range(self.n_step - 1):
                for dim in range(self.n_system_vars):
                    ...
                    for i in range(self.n_order - 1):
                        ...
                        self.add_constraint(
                            var_list=var_list, values=val_list, rhs=0, constraint_type=ConstraintType.Derivative
                        )
    ...
    def build_constraints(self):
        ...
        eq_A = torch.sparse_coo_tensor(
            [self.row_dict[ConstraintType.Equation], self.col_dict[ConstraintType.Equation]],
        ...
        if self.n_iv > 0:
            initial_A = torch.sparse_coo_tensor(
                [self.row_dict[ConstraintType.Initial], self.col_dict[ConstraintType.Initial]],
        ...
        derivative_A = torch.sparse_coo_tensor(
            [self.row_dict[ConstraintType.Derivative], self.col_dict[ConstraintType.Derivative]],
    ...
    def get_row_col_sorted_indices(self, row, col, exclude_eps=True):
        """Compute indices sorted by row and column and repeats.
        Useful for sparse outer product when computing constraint derivatives"""
    ...
    def set_row_col_sorted_indices(self):
        ...
        row_sorted, column_sorted, row_counts, column_counts = self.get_row_col_sorted_indices(
            derivative_rows, derivative_columns
        )
    ...
    def sparse_grad_derivative_constraint(self, x, y):
        ...
        x = x[
            :,
            self.num_added_equation_constraints
            + self.num_added_initial_constraints : self.num_added_equation_constraints
            + self.num_added_initial_constraints
            + self.num_added_derivative_constraints,
        ]
        y = y[:, : self.num_vars]
